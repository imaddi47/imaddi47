...
| `pnpm sync:writing` | Reads the blog RSS feed (`blog.debugblackbox.com/feed`) and prints a **paste-ready `writings` snippet** with your latest posts (title, date, excerpt, link). |
...
- **Writing** — run `pnpm sync:writing`, review, and paste the latest posts into the `writings` array.
